...
const EmailForm = ({setShowThankYou, setShowFindForm, dataUser, setDataUser, showEmailForm, setShowEmailForm, emailData, setEmailData, clientId}) => {
    ...
    const send = async e => {
        ...
        const payload = await axios.post(`https://payload-demo-tpm.herokuapp.com/state-email?to=${emailData.contact}&subject=${dataUser.subject}&clientId=${clientId}&state=${dataUser.state}&firstName=${dataUser.userName}&emailData=${dataUser.emailUser}&text=${dataUser.text.replace(/\n\r?/g, "<br/>")}`)
